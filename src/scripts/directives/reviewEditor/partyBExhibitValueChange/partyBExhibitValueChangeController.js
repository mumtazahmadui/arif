(function() {
    ...
    angular.module('app.directives')
        ...
        function(AmendmentLetter, $scope, tableWithLineBreaks, validataTable, prepareDataToSend) {
            ...
            $scope.isDefaultFieldsFoxExhibit = function(name) {
            ...
            function load() {
                return AmendmentLetter
                    .getPartyBExhibitValueChangePlaceHolder($scope.amendmentId).then(function(data) {
            ...
            function render(data) {
                $scope.partyBExhibitValueChange = tableWithLineBreaks.createTableData(data);
                $scope.isShowHeader = !Object.keys($scope.partyBExhibitValueChange.tables).length;
            }

            function onSave(event, params) {
                var sendData = prepareDataToSend.buildData($scope.partyBExhibitValueChange.tables);
                $scope.loaded = false;
                $scope.notificationText = 'Saving...';
                AmendmentLetter
                    .updateBExhibitValue($scope.amendmentId, sendData)
                    .success(params.success)
                    .error(params.error);
            }

            var isValid = function() {
                if (!validataTable.validate($scope.partyBExhibitValueChange.tables)) {
