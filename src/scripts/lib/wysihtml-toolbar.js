...
**/
    ...
    var removeWordMetadata = function(content){
        // Meta and link tags.
...
wysihtml5.dom.getPastedHtml = function(event) {
  ...
  if (wysihtml5.browser.supportsModernPaste() && event.clipboardData) {
    if (wysihtml5.lang.array(event.clipboardData.types).contains('text/html')) {
      var t = getDataTransferItems(event.clipboardData)
      html = removeWordMetadata(t["text/html"]);
      ...
      dom.observe(this.element, "keydown", function(event) {
        ...
        if (blockElement) {
          setTimeout(function() {
            ...
            var browsers = !/chrome/.test(userAgentLowerCase) && !/mozilla/.test(userAgentLowerCase)
            var isRunAdj = keyCode === wysihtml5.ENTER_KEY && (blockElement.nodeName.match(/^H[1-6]$/) || browser);
            if (isRunAdj) {
              adjust(selectedNode);
              setEndOfContenteditable(selectedNode);
            }
          }, 0);
          return;
        }
...
(function(wysihtml5) {
  ...
  wysihtml5.views.Composer.prototype.observe = function() {
    ...
    addListeners(this.element, ["drop", "paste"], handlePaste.bind(this), false);
